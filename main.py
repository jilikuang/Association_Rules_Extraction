...
def display(itemsets, rules, min_supp, min_conf):
    ...
    # print itemsets
    for s in itemsets:
        print '[' + ', '.join(s[0]) + '], ' + '{:.4f}'.format(s[1]*100.0) + '%'
    print
    print '==High-confidence association rules (min_conf=' + str(min_conf*100.0) + '%)'
    # print rules
    for r in rules:
        print '[' + ', '.join(r[0]) + '] => [' + ', '.join(r[1]) + '], (Conf: ' + '{:.4f}'.format(r[2]*100.0) + '%, Supp: ' + '{:.4f}'.format(r[3]*100.0) + '%)'
...
def output(itemsets, rules, min_supp, min_conf):
    ...
    # print itemsets
    for s in itemsets:
        out.write('[' + ', '.join(s[0]) + '], ' + '{:.4f}'.format(s[1]*100.0) + '%\n')
    out.write('\n')
    out.write('==High-confidence association rules (min_conf=' + str(min_conf*100.0) + '%)\n')
    # print rules
    for r in rules:
        out.write('[' + ', '.join(r[0]) + '] => [' + ', '.join(r[1]) + '], (Conf: ' + '{:.4f}'.format(r[2]*100.0) + '%, Supp: ' + '{:.4f}'.format(r[3]*100.0) + '%)\n')
    out.close()
